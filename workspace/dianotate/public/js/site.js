...
function createCallout(id, target, text)
{
	...
	$("body").append("<div gs='true' style='position:absolute' id='" + id + "'>" + 
	...
	$('#' + id).on("mouseup", function(event)
			{
				...
				ignoreMouseup = true;
				console.log("Drop repositioning");
				...
				for(var i=0;i< window.tracks.length;i++)
				{
					...
					if(track.attachment ==  id)
					{
						...
						track.offset = {left: left, top: top};
					}
				}
			}); 
}
